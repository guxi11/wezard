...
export const renderSchedule = (s: Schedule, now: Date = new Date()): Record<string, unknown> => ({
  ...
  note: s.note,
  runIn: s.fresh ? "每次新建一个白板 wizard 执行, 跑完自动回收" : "注入已有会话",
  target: s.target,
  prompt: s.prompt,
});
...
export const startScheduler = ({ client, cfg, sourcePath, log, inject }: SchedulerDeps): { stop: () => void } => {
  const fire = async (s: Schedule): Promise<void> => {
    ...
    // 冒出一轮对话, 没人知道是谁点的火。
    const how = s.fresh ? " · 新建 wizard 执行" : "";
    await announce(client, s.target, `⏰ **定时任务** · ${describeWhen(s.when)}${how}\n> ${s.prompt.split("\n")[0]!.slice(0, 120)}`)
      .catch((e: unknown) => log.warn({ id: s.id, err: (e as Error).message }, "task announce failed"));
    const r = await inject(s.target, s.prompt, { taskId: s.id, fresh: s.fresh });
