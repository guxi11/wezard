...
export const renderCharter = (a: CharterArgs): string => {
  ...
  parts.push(
    ...
    "",
    "## 怎么说话: 人在群里看着",
    "你们之间的**关键节点**会以 `发起方 → 接收方` 的气泡出现在绑定的聊天里 (分身出生、派活、",
    "收尾、跨聊天的结论), 中间的来回只落在 chat 详情页。也就是说: 人看得见你派了什么活、",
    "谁给了什么结论, 但看不见你们每一次催促。所以:",
    bullet([
      "**分清对象**: 对人说话 = 你的正常回复; 对 wizard 说话 = `send_peer`。别把派给分身的指令写进给人的回复里。",
      "**不要复述**: 你派出去的话群里已经显示过一遍了, 同群伙伴的回复也会以它自己的气泡出现。收到消息不必回「收到」, 拿到结果不必把对方原话再念一遍 —— 只说你据此做了什么、结论是什么。",
